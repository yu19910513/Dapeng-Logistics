...
///// loading functions ///////
async function archive(container_number) {
    const response = await fetch(`/api/container/client_archive/${container_number}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' }
      });
      if (response.ok) {
        console.log(`${container_number} is temporarily archived!`)
      } else {
        alert('try again')
    }
}
